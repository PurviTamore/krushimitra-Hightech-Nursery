...
function HomePage() {
  ...
  // Use simple string paths starting with "/" - this points to the public folder
  const items = [
      ...
      {!showCarousel ? (
        ...
        <div className="content" style={{ paddingTop: '80px' }}>
          <div className="carousel">
            <div className="carousel-wrapper" style={{ height: '420px', marginBottom: '30px' }}>
              {items.map((item, index) => {
                let className = "card";
                if (index === activeIndex) className += " active";
                else if (index === (activeIndex - 1 + items.length) % items.length) className += " prev";
                else if (index === (activeIndex + 1) % items.length) className += " next";
